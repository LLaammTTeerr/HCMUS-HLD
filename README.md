...
HCMUS-HLD algorithms should be: useful, short, fast enough, well tested, and if relevant, readable and easy to modify.
...
## Customizing HCMUS-HLD

While HCMUS-HLD is usable as is, it's also easy to modify if you want to create a personalized copy.
...
`content/hcmus.tex` is the main file of HCMUS-HLD, and can be edited to change team name, logo, syntax highlighting, etc.
...
HCMUS-HLD uses a relatively terse coding style, with a handful of macros/typedefs defined in the
...
HCMUS-HLD aims for a high level of confidence in algorithm correctness.
