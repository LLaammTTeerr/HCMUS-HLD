 ...
 * Solution - Let's reformulate the restrictions. For a restriction (l(i),r(i),x(i),c(i)) We can assume that we will get punished for all of them and that we will get our c(i)
 ...
 represents reducing the i-th from height h to h−1 for the cost h^2 − (h−1)^2. The project (i, h) is clearly dependent on the project (i, h + 1). Let's also add our 
 restrictions as projects. A restriction (l(i),r(i),x(i),c(i)) is equivalent to a project with profit c(i) dependent on the projects (l(i),x(i)), (l(i)+1,x(i)) … (r(i),x(i)). 
 ...
 * Solution - 
    ...
    - For every edge left - right build edge left -> right (cap = +oo)
 ...
 * Solution - 
    ...
    - For each edge u -> v with flow in range [L, R], we build edge:
        - (u -> v) with cap = R - L
        - (u -> Tp) with cap = L
        - (Sp -> v) with cap = L
    
    - Then we call max flow from Sp to Tp. if (max flow from Sp to Tp) < (sum of all L of edge) then L-R flow doesn't exist.
    - If not, the answer is max flow from S to T.
*/